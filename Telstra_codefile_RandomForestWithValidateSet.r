...
predknown=predict(rf_model,newdata=Actual_Validateset,type='raw')
...
pred=predict(rf_model,newdata=X_test[,feature.namestest],type='raw')

##making submission file
mysub=data.frame(id=X_test$id,predclass=pred)
mysub1=mysub %>% mutate(i=1) %>% spread(predclass,i,fill=0)
colnames(mysub1)=c("id","predict_0","predict_1","predict_2")

write.csv(mysub1,'E:\\AbhinavB\\Kaggle\\Telstra network disruptions\\submission files\\submission2_Jan30_randomForest.csv')
